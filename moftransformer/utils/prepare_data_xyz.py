...
def _split_dataset(root_dataset: Path, **kwargs):
    ...
    for struct_file in list(total_dir.glob("*.cif")) + list(total_dir.glob("*.xyz")) + list(total_dir.glob("*.extxyz")):
        cif_id = struct_file.stem
        graphdata = struct_file.with_suffix(".graphdata")
        grid = struct_file.with_suffix(".grid")
        griddata = struct_file.with_suffix(".griddata16")

        if struct_file.exists() and graphdata.exists() and grid.exists() and griddata.exists():
            cif_list[cif_id] = CifPath(struct_file, graphdata, grid, griddata)
    ...
    for split, direc in split_dir.items():
        split_cifs = {f.stem for ext in ["*.cif", "*.xyz", "*.extxyz"] for f in direc.glob(ext)}
...
def _split_json(root_cifs: Path, root_dataset: Path, downstream: str):
    with open(str(root_cifs / f"raw_{downstream}.json")) as f:
        src = json.load(f)
        src = {
            i.replace(".cif", "").replace(".xyz", "").replace(".extxyz", ""): v for i, v in src.items()
        }  # if structure extension in JSON files

    for split in ["train", "test", "val"]:
        cif_folder = root_dataset / split
        cif_list = [f.stem for ext in ["*.cif", "*.xyz", "*.extxyz"] for f in cif_folder.glob(ext)]
...
def make_prepared_data(
    ...
    if not any(atoms.pbc):
        logger.error(f"{cif_id} failed : No Periodic Boundary Conditions (PBC) found.")
        return False
        
    if np.allclose(atoms.cell.cellpar()[:3], 0):
        logger.error(f"{cif_id} failed : Cell lengths are zero. Please check PBC and cell parameters.")
        return False
    ...
    if eg_success:
        ...
        save_cif_path = root_dataset_total / f"{cif_id}{cif.suffix}"
...
def prepare_data(root_cifs, root_dataset, downstream, **kwargs):
    ...
    file_list = list(root_cifs.glob("*.cif")) + list(root_cifs.glob("*.xyz")) + list(root_cifs.glob("*.extxyz"))
    for cif in tqdm(
